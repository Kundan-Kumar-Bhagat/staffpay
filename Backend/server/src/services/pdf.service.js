...
export function payslipPDF(p, company, user) {
  ...
  if (p.source === 'manual') {
    doc.rect(M, y, CW, 18).fill('#FBEED7');
    doc.rect(M, y, 3, 18).fill(AMBER);
    doc.fillColor('#8F621F').font('Helvetica-Bold').fontSize(7.4)
      .text(`MANUALLY ISSUED by ${p.generatedBy?.name || 'management'}${p.note ? ' — ' + p.note : ''}. Figures entered by hand, not derived from attendance.`, M + 12, y + 5.5, { width: CW - 24 });
    y += 26;
  }
...
export function voucherPDF(v, company) {
  const cur = company.currency;
  const accent = brandAccent(company);
  const logo = loadLogo(company);
  ...
  doc.rect(0, 96, W, 4).fill(AMBER);
  let nx = M;
  if (logo) { try { doc.image(logo, M, 20, { fit: [52, 52] }); nx = M + 62; } catch {} }
  doc.fillColor('#FFFFFF').font('Helvetica-Bold').fontSize(18).text(company.name, nx, 22, { width: 320 });
  doc.fontSize(8.6).fillColor('#DDE8E2');
  doc.text(`${company.address}, ${company.city}, ${company.state} ${company.zip}${company.taxId ? ' • GSTIN: ' + company.taxId : ''}`, nx, 44, { width: 320 });
  doc.text(`${company.email} • ${company.phone}`, nx, 56, { width: 320 });
  doc.fillColor('#FFFFFF').font('Helvetica-Bold').fontSize(19).text('PAYMENT VOUCHER', W - M, 22, { align: 'right' });
  doc.fillColor(AMBER).font('Helvetica-Bold').fontSize(11).text(v.number, W - M, 48, { align: 'right' });
  doc.fillColor('#DDE8E2').font('Helvetica').fontSize(8.4).text(`Status: ${v.status.toUpperCase()}  •  ${v.paymentMode.toUpperCase()}`, W - M, 64, { align: 'right' });

  let y = 122;
  const colW = CW / 2 - 14;
  kv(doc, 'Paid To', `${v.payee.name}${v.payee.phone ? '\n' + v.payee.phone : ''}${v.payee.address ? '\n' + v.payee.address : ''}`, M, y, colW);
  kv(doc, 'Voucher Details',
    `Date: ${new Date(v.createdAt).toLocaleDateString('en-GB')}${v.from ? '\nWork period: ' + v.from + (v.to ? ' → ' + v.to : '') : ''}${v.payee.idType ? '\n' + v.payee.idType + ': ' + (v.payee.idNumber || '—') : ''}`,
    M + colW + 28, y, colW);
  y += 82;

  doc.rect(M, y, CW, 22).fill(accent);
  doc.fillColor('#FFFFFF').font('Helvetica-Bold').fontSize(8.2);
  doc.text('DESCRIPTION OF WORK / SERVICE', M + 10, y + 7, { width: CW * 0.5 });
  doc.text('QTY', M + CW * 0.56, y + 7, { width: CW * 0.1 });
  doc.text('RATE', M + CW * 0.68, y + 7, { width: CW * 0.14, align: 'right' });
  doc.text('AMOUNT', W - M - 10, y + 7, { width: CW * 0.17, align: 'right' });
  y += 22;
  doc.rect(M, y, CW, 22).fill(SOFT);
  doc.fillColor(INK).font('Helvetica').fontSize(9.5);
  doc.text(v.description || '—', M + 10, y + 7, { width: CW * 0.5 });
  doc.text(String(v.qty), M + CW * 0.56, y + 7, { width: CW * 0.1 });
  doc.text(fmtMoney(v.rate, cur), M + CW * 0.68, y + 7, { width: CW * 0.14, align: 'right' });
  doc.text(fmtMoney(v.amount, cur), W - M - 10, y + 7, { width: CW * 0.17, align: 'right' });
  y += 32;

  const line = (label, val, bold) => {
    doc.fillColor(INK).font(bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(bold ? 11 : 9.5);
    doc.text(label, W - M - 230, y, { width: 140 });
    doc.text(fmtMoney(val, cur), W - M - 10, y, { width: 80, align: 'right' });
    y += bold ? 22 : 16;
  };
  line('Gross Amount', v.amount);
  if (v.deductions.tds) line('Less: TDS', -v.deductions.tds);
  if (v.deductions.advance) line('Less: Advance', -v.deductions.advance);
  if (v.deductions.other) line('Less: Others', -v.deductions.other);
  doc.moveTo(W - M - 230, y - 4).lineTo(W - M, y - 4).strokeColor(INK).lineWidth(1).stroke();
  doc.rect(W - M - 236, y, 236, 28).fill(accent);
  doc.fillColor('#FFFFFF').font('Helvetica-Bold').fontSize(11);
  doc.text('NET PAYABLE', W - M - 226, y + 9);
  doc.text(fmtMoney(v.net, cur), W - M - 12, y + 9, { align: 'right' });
  y += 40;

  doc.fillColor(MUTED).font('Helvetica-Bold').fontSize(7).text('AMOUNT IN WORDS', M, y);
  doc.fillColor(INK).font('Helvetica').fontSize(9.5).text(amountInWords(v.net, cur), M, y + 10, { width: CW });
  y += 34;
  if (v.note) { doc.fillColor(MUTED).fontSize(8.5).text(`Note: ${v.note}`, M, y, { width: CW }); y += 18; }

  const sigY = y + 28;
  doc.moveTo(M, sigY).lineTo(M + 160, sigY).strokeColor(INK).lineWidth(0.8).stroke();
  doc.fillColor(INK).font('Helvetica-Bold').fontSize(9).text('Payee signature (payment received)', M, sigY + 5);
  doc.moveTo(W - M - 160, sigY).lineTo(W - M, sigY).strokeColor(INK).lineWidth(0.8).stroke();
  doc.fillColor(INK).font('Helvetica-Bold').fontSize(9).text(company.managerName || 'Authorised signatory', W - M, sigY + 5, { align: 'right' });
  doc.fillColor(MUTED).font('Helvetica').fontSize(7.6).text(company.managerTitle || 'Manager', W - M, sigY + 16, { align: 'right' });

  doc.rect(0, doc.page.height - 40, W, 40).fill(SOFT);
  doc.fillColor(MUTED).font('Helvetica').fontSize(6.8)
    .text(`This is a payment voucher for contractual / casual / one-time work and does not constitute employment or a salary relationship. Verify authenticity at ${company.website || 'the StaffPay portal'}/verify using voucher no. ${v.number}.`, M, doc.page.height - 32, { width: CW });
  doc.end();
  return doc;
}
