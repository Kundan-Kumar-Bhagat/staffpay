...
export async function computeAndSavePayslip(user, month, company, byId, customOverrides = null) {
  ...
  if (customOverrides) {
    ...
    earnings = {
      basic: Math.round(+e.basic || 0),
      hra: Math.round(+e.hra || 0),
      allowances: Math.round(+e.allowances || 0),
      overtime: Math.round(+e.overtime || 0),
    };
    deductions = {
      pf: Math.round(+d.pf || 0),
      tax: Math.round(+d.tax || 0),
      absent: Math.round(+d.absent || 0),
      unpaidLeave: Math.round(+d.unpaidLeave || 0),
      advance: Math.round(+d.advance || 0),
    };
    days = {
      working: +dy.working || 26,
      present: +dy.present || 26,
      late: +dy.late || 0,
      absent: +dy.absent || 0,
      leave: +dy.leave || 0,
      half: +dy.half || 0,
  ...
  return Payslip.findOneAndUpdate(
    { user: user._id, month },
    {
      ...
      days, status: 'issued', generatedBy: byId,
    },
