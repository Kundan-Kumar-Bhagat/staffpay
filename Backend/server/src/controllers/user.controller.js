...
import { seatLimit } from '../config/plans.js';
...
export const create = async (req, res) => {
  ...
  const b = ws.billing || {};
  if (ws.plan === 'trial' && b.status === 'expired')
    return res.status(402).json({ message: 'Your trial has ended — upgrade from Billing to keep adding staff' });
  const limit = seatLimit(ws);
  if ((await User.countDocuments()) >= limit)
    return res.status(402).json({ message: ws.plan === 'pro'
      ? `All ${limit} purchased seats are in use — add seats from Billing.`
      : `The trial plan caps at ${limit} staff. Upgrade from Billing for unlimited seats.` });
